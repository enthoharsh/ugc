...
const BrandRegisterForm = (props) => {
	const { showLoading, signUp, loading } = props;
	...
	const rules = {
	  ...
	  businessName: [{ required: true, message: 'Please input your business name' }],
	  industry: [{ required: true, message: 'Please select your industry' }],
	  brandName: [{ required: true, message: 'Please input your brand/agency name' }],
	  companyType: [{ required: true, message: 'Please select your company type' }],
	  companyIndustry: [{ required: true, message: 'Please select your company industry' }],
	  annualRevenue: [{ required: true, message: 'Please select your annual revenue' }],
	  termsAccepted: [{ 
		validator: (_, value) => value ? Promise.resolve() : Promise.reject('Please accept the terms of service'),
	  }]
	};
	...
	const onFinish = async (values) => {
	  try {
		...
		// First call your custom API here
		// await customApiCall(finalFormData);
		
		// Then call the default signup
	...
	const getFieldsForStep = (step) => {
	  switch (step) {
		...
		case 2:
		  return ['companyType', 'companyIndustry', 'annualRevenue', 'termsAccepted'];
		default:
		  return [];
	  }
	};
	...
	const renderBusinessInfo = () => (
	  <>
		...
		<Form.Item name="brandName" label="Brand/Agency Name" rules={rules.brandName}>
		  <Input prefix={<BankOutlined className="text-primary" />} />
		</Form.Item>
	  </>
	);
	...
	const renderCompanyDetails = () => (
	  <>
		<Form.Item name="companyType" label="Company Type" rules={rules.companyType}>
		...
		<Form.Item name="companyIndustry" label="Company Industry" rules={rules.companyIndustry}>
		...
		<Form.Item name="annualRevenue" label="Annual Revenue" rules={rules.annualRevenue}>
		...
		<Form.Item name="termsAccepted" valuePropName="checked" rules={rules.termsAccepted}>
		  <Checkbox>
